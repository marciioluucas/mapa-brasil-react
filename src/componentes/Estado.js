...
export default function Estado({estado, onClick, ativo, activeColor, backgroundColor, labelColor, strokeColor}) {
	...
	function getCustomCssColor(color) {
		const colours = {
			"aliceblue": "#f0f8ff",
			"antiquewhite": "#faebd7",
			"aqua": "#00ffff",
	...
	const contrastText = useCallback(()=> {
		let background = backgroundColor;
		const customColor = getCustomCssColor(background);

		if (!!customColor) background = customColor;

		const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(background);
	...
	return (
		...
		>
			<title>{nome}</title>
			<Path $backgroundColor={cor} $strokeColor={!!strokeColor ? strokeColor : contrastText()} d={coordenadas[0]}/>
			<Text transform={transform} $labelColor={!!labelColor ? labelColor : contrastText()}>{sigla}</Text>
		</Link>
	)
}